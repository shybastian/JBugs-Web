...
var BugDispatch = {
  ...
  initialize: function (options) {
    ...
    if ('transform' in document.getElementById(this.options.elementId).style) {
      ...
      for (i = 0; i < vendors.length; i++) {
        if (vendors[i] + 'Transform' in document.getElementById(this.options.elementId).style) {
    ...
    for (i = 0; i < numBugs; i++) {
      ...
      bugsIn = window.setTimeout((function (thebug) {
        return function () {
          thebug.flyIn();
        };
      }(thebug)), delay);
  ...
  check_if_mouse_close_to_bug: function (e) {
    ...
    if (e.client && e.client.x) {
      ...
      posx = e.page.x - (document.getElementById(this.options.elementId).scrollLeft + document.getElementById(this.options.elementId).scrollLeft);
      posy = e.page.y - (document.getElementById(this.options.elementId).scrollTop + document.getElementById(this.options.elementId).scrollTop);
    } else if (e.pageX) {
      posx = e.pageX - (document.getElementById(this.options.elementId).scrollLeft + document.getElementById(this.options.elementId).scrollLeft);
      posy = e.pageY - (document.getElementById(this.options.elementId).scrollTop + document.getElementById(this.options.elementId).scrollTop);
    }
...
var Bug = {
  ...
  setPos: function (top, left) {
    this.bug.top = top || this.random(this.options.edge_resistance, document.getElementById(this.options.elementId).clientHeight - this.options.edge_resistance);
    //bug.top = bug.top < 0 ? -bug.top : bug.top;
    this.bug.left = left || this.random(this.options.edge_resistance, document.getElementById(this.options.elementId).clientWidth - this.options.edge_resistance);
  ...
  drawBug: function (top, left) {
    ...
    if (!this.inserted) {
      this.inserted = true;
      document.getElementById(this.options.elementId).appendChild(this.bug);
    }
  },
  ...
  flyRand: function () {
    this.stop();
    var landingPosition = {};
    landingPosition.top = this.random(this.options.edge_resistance, document.getElementById(this.options.elementId).clientHeight - this.options.edge_resistance);
    landingPosition.left = this.random(this.options.edge_resistance, document.getElementById(this.options.elementId).clientWidth - this.options.edge_resistance);
  ...
  flyIn: function () {
    ...
    var side = Math.round(Math.random() * 4 - 0.5),
      d = document,
      e = d.getElementById(this.options.elementId),
      g = d.getElementById(this.options.elementId),
      windowX = e.innerWidth || e.clientWidth || g.clientWidth,
      windowY = e.innerHeight || e.clientHeight || g.clientHeight;
    if (side > 3) side = 3;
    if (side < 0) side = 0;
    var style = {}, s;
    if (side === 0) {
  ...
  drop: function (deathType) {
    var startPos = this.getPos().top,
      ...
      e = d.getElementById(this.options.elementId),
      g = d.getElementById(this.options.elementId),
      finalPos = e.innerHeight || e.clientHeight || g.clientHeight,
  ...
  bug_near_window_edge: function () {
    ...
    else if (this.bug.top > document.getElementById(this.options.elementId).clientHeight - this.options.edge_resistance)
      this.near_edge |= this.NEAR_BOTTOM_EDGE;
    if (this.bug.left < this.options.edge_resistance)
      this.near_edge |= this.NEAR_LEFT_EDGE;
    else if (this.bug.left > document.getElementById(this.options.elementId).clientWidth - this.options.edge_resistance)
